// src/modules/audit-results-findings/document-adherence/fm/providers/fetchProviders.ts
...
export async function fetchSubsystemRecords(
  ...
  params.set("subsystem", "fm");          // ← always filter to FM
  if (docType)   params.set("docType",    docType);
  if (user)      params.set("preparedBy", user);
  if (startDate) params.set("startDate",  startDate);
  if (endDate)   params.set("endDate",    endDate);

  const url = `/api/arf/document-adherance/fm?${params}`;
