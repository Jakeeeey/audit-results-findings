...
type Props = {
    ...
    onOpen: (row: MockLedgerListRow) => void;
};
...
function StatusBadge({ status }: { status: MockLedgerListRow["status"] }) {
    if (status === "Committed") {
    ...
    return (
        <div className="flex min-h-[220px] items-center justify-center rounded-2xl border border-dashed bg-muted/20 p-8 text-center">
            <div className="space-y-2">
                <p className="text-sm font-medium">No Mock Ledger records found</p>
...
export function MockLedgerListTable(props: Props) {
    const {
        rows,
        isLoading,
