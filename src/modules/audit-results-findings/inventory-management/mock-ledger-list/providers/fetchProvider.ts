import type {
    ...
    PriceTypeRow,
    SupplierRow,
} from "@/modules/audit-results-findings/inventory-management/physical-inventory-mock-ledger";
import {
    derivePhysicalInventoryStatus,
    fetchBranches,
    fetchCategories,
    fetchMockLedgerList as fetchPhysicalInventoryList,
    fetchPriceTypes,
    fetchSuppliers,
    normalizeFlagValue,
} from "@/modules/audit-results-findings/inventory-management/physical-inventory-mock-ledger";

import type { MockLedgerListRow } from "../types";

export async function fetchMockLedgerListRows(): Promise<{
    rows: MockLedgerListRow[];
    branches: BranchRow[];
    ...
    const rows: MockLedgerListRow[] = headers.map(
        (header: MockLedgerHeaderRow): MockLedgerListRow => {
