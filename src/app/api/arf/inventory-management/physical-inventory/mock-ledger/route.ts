...
import type { MockLedgerHeaderRow as PhysicalInventoryHeaderRow } from "../../../../../../modules/audit-results-findings/inventory-management/physical-inventory-mock-ledger/types";
...
export async function POST(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        const created = await directusFetch<PhysicalInventoryHeaderRow & { ml_no?: string | null }>(
            "/items/mock_ledger",
            {
                method: "POST",
                body: JSON.stringify({
        ...
        const mappedCreated = { ...created, ph_no: created.ml_no ?? created.ph_no };
        delete mappedCreated.ml_no;
        return NextResponse.json(mappedCreated);
    } catch (error) {
        return NextResponse.json(
            {
                message:
