// src/app/api/arf/document-adherance/crm/route.ts
...
export async function GET(request: NextRequest) {
  ...
  // Filter to only CRM subsystem records since backend doesn't support subsystem param
  const allData = json.data ?? [];
  const crmData = allData.filter(
    (record) => typeof record.subsystem === 'string' && record.subsystem.toLowerCase() === 'crm'
  );
  ...
  try {
    ...
    console.error('[CRM API] Failed to fetch NTEs:', err);
  }
  ...
  try {
    ...
    console.error('[CRM API] Failed to fetch Remarks:', err);
  }

  // 3. Merge
  const mergedData = crmData.map(record => {
