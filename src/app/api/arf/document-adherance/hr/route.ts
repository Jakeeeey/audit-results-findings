// src/app/api/arf/document-adherance/hr/route.ts
...
export async function GET(request: NextRequest) {
  ...
  // Filter to only HR subsystem records since backend doesn't support subsystem param
  const allData = json.data ?? [];
  const hrData = allData.filter(
    (record) => typeof record.subsystem === 'string' && record.subsystem.toLowerCase() === 'hr'
  );
  ...
  try {
    ...
    console.error('[HR API] Failed to fetch NTEs:', err);
  }
  ...
  try {
    ...
    console.error('[HR API] Failed to fetch Remarks:', err);
  }

  // 3. Merge
  const mergedData = hrData.map(record => {
