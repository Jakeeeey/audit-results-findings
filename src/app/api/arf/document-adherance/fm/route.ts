// src/app/api/arf/document-adherance/fm/route.ts
...
export async function GET(request: NextRequest) {
  ...
  // Filter to only FM subsystem records since backend doesn't support subsystem param
  const allData = json.data ?? [];
  const fmData = allData.filter(
    (record) => typeof record.subsystem === 'string' && record.subsystem.toLowerCase() === 'fm'
  );
  ...
  try {
    ...
    console.error('[FM API] Failed to fetch NTEs:', err);
  }
  ...
  try {
    ...
    console.error('[FM API] Failed to fetch Remarks:', err);
  }

  // 3. Merge
  const mergedData = fmData.map(record => {
