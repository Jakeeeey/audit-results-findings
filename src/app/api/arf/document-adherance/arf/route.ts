// src/app/api/arf/document-adherance/arf/route.ts
...
export async function GET(request: NextRequest) {
  ...
  // Filter to only ARF subsystem records since backend doesn't support subsystem param
  const allData = json.data ?? [];
  const arfData = allData.filter(
    (record) => typeof record.subsystem === 'string' && record.subsystem.toLowerCase() === 'arf'
  );
  ...
  try {
    ...
    console.error('[ARF API] Failed to fetch NTEs:', err);
  }
  ...
  try {
    ...
    console.error('[ARF API] Failed to fetch Remarks:', err);
  }

  // 3. Merge
  const mergedData = arfData.map(record => {
