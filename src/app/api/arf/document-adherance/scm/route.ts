// src/app/api/arf/document-adherance/scm/route.ts
...
export async function GET(request: NextRequest) {
  ...
  // Filter to only SCM subsystem records since backend doesn't support subsystem param
  const allData = json.data ?? [];
  const scmData = allData.filter(
    (record) => typeof record.subsystem === 'string' && record.subsystem.toLowerCase() === 'scm'
  );
  ...
  try {
    ...
    console.error('[SCM API] Failed to fetch NTEs:', err);
  }
  ...
  try {
    ...
    console.error('[SCM API] Failed to fetch Remarks:', err);
  }

  // 3. Merge
  const mergedData = scmData.map(record => {
